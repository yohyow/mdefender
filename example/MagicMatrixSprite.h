...
public:
	  ...
	  // 注意一点图片必须放在缓存里面
	  static MagicMatrixSprite* createWithPic(const char* pMagicPic,const char* pMagicPicBg); // 第一个参数CD 的比较暗淡的图片 第二个是比亮的图片
	  CC_SYNTHESIZE(float,mana,Mana);// 消耗魔法值
	  void runMagicCDAnimation();// 执行魔法CD 动画
	  CC_SYNTHESIZE(const char* ,speciaPicName,SpeciaPicName);// 图片的名字
	  CC_SYNTHESIZE(const char* ,speciaFileName,SpeciaFileName);// plist 文件的名字
	  CC_SYNTHESIZE(const char* ,speciaName,SpeciaName);//特效的通用名字
